...
from evidential_regression.training.losses import nig_nll
...
def train_and_val_proc(
    ...
    lamb_loss : float, optional
        Coefficient for regularization in the loss function, by default 0.01.
    ...
    for epoch in range(epochs):
        ...
        for X_train, y_train in dataloader_dict["train"]:
            ...
            train_loss = criterion(y_train_pred_dist_params, y_train, lamb=lamb_loss)
            train_loss.backward()
        ...
        with torch.no_grad():
            for X_val, y_val in dataloader_dict["val"]:
                ...
                val_loss = nig_nll(*y_val_pred_dist_params, y_val)
                mu, _, _, _ = [
        ...
        if early_stopper.early_stop(val_rmse, model, epoch):
            ...
            if best_model_save_path and early_stopper.best_model_state:
                ...
                print(
                    f"Best model state recorded at epoch {early_stopper.best_epoch+1} with RMSE: {early_stopper.best_metric:.3f} (no model saved as path_to_save was None)."
                )
            break
    ...
    if not early_stopper.early_stop_triggered:
        if best_model_save_path and early_stopper.best_model_state:
            ...
            print(
                f"Training finished. Best validation RMSE: {early_stopper.best_metric:.3f} at epoch {early_stopper.best_epoch+1} (no model saved as path_to_save was None)."
            )
...
def test_proc(
    ...
    """
    Evaluate a trained PyTorch model on a test dataset and save metrics to CSV.
    ...
    dataloader_dict : dict
        Dictionary containing DataLoader for the 'test' dataset.
    device : torch.device
        Device on which computations are performed (CPU or GPU).
    path_to_save : Path
    ...
    target_inverse_transform : callable, optional
        Function to inverse-transform target values and predictions, by default None.
    key: str
        Determine evaluation on test or ext test, by default test.
    """
    model.eval()
    test_losses = []
    y_test_all = []
    ...
    with torch.no_grad():
        for X_test, y_test in dataloader_dict[key]:
            ...
            y_test_pred_dist_params = model(X_test)
            test_loss = criterion(*y_test_pred_dist_params, y_test)
            mu, nu, alpha, beta = [
                d.squeeze().detach().cpu().numpy() for d in y_test_pred_dist_params
            ]
            test_losses.append(test_loss.item())
            y_test_all.append(y_test.detach().cpu().numpy())
            y_test_pred_all.append(mu)

            aleatoric_unc = beta / (alpha - 1)
            epistemic_unc = beta / (nu * (alpha - 1))
    ...
    un_results_path = path_to_save / f"results_enn_{key}_set.csv"
...
def val_proc(
    ...
    """
    Evaluate a trained PyTorch model on a validation dataset and save metrics to CSV.
    ...
    un_results_path = path_to_save / "results_enn_val_set.csv"
    un_results_df.to_csv(un_results_path, index=False)
